defmodule LeroWeb.MessageControllerTest do
  ...
  describe "authorized" do
    ...
    test "get conversation messages with direction", %{conn: conn} do
      ...
      assert %{ "success" => true, "messages" => messages, "page" => page, "total" => total } = json_response(conn, 200)
    ...
    test "get conversation messages comes paginated from 20 last", %{conn: conn} do
      conversation = conversation_with_messages_fixture(15, 13)
      conn = get(conn, Routes.conversation_path(conn, :details, conversation.id))
    ...
    test "get conversation messages (8) from page 1 for 28 messages", %{conn: conn} do
      conversation = conversation_with_messages_fixture(15, 13)
      conn = get(conn, Routes.conversation_path(conn, :details, conversation.id), page: 1)
